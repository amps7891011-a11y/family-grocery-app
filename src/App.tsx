...
function PinScreen({onUnlock}:{onUnlock:()=>void}) {
  ...
  const submit=async()=>{
    ...
    setLoading(true);
    try{
      const res=await dbGet(PIN_KEY_DB);
      ...
      <div style={{background:"white",borderRadius:24,padding:"2.5rem 2rem",width:"100%",maxWidth:360,boxShadow:"0 20px 60px rgba(0,0,0,0.2)",textAlign:"center",animation:shake?"shake 0.4s ease":"none"}}>
        ...
        {lockout>0?(
          ...
          <>
            ...
            <button onClick={submit} disabled={loading} style={{width:"100%",padding:"14px",borderRadius:12,border:"none",background:"linear-gradient(135deg,#4F46E5,#7C3AED)",color:"white",fontSize:16,fontWeight:600,cursor:loading?"wait":"pointer",boxShadow:"0 4px 12px rgba(79,70,229,0.4)",opacity:loading?0.8:1}}>
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:"1rem",backdropFilter:"blur(4px)"}}>
      <div style={{background:"white",borderRadius:20,padding:"1.5rem",width:"100%",maxWidth:340,boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
        ...
        <div style={{fontSize:12,color:"#6B7280",background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:8,padding:"8px 12px",marginBottom:14}}>✓ PIN will update on all devices instantly</div>
        {(["current","next","confirm"] as const).map((key,i)=>(
          <div key={key} style={{marginBottom:12}}>
            ...
            <input type="password" placeholder={["Enter current PIN","Min 4 characters, alphanumeric OK","Re-enter new PIN"][i]}
    ...
    <div style={{minHeight:"100vh",background:"#F8FAFF",fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      <style>{`
        *{box-sizing:border-box;}
        ...
        .tab-btn{background:none;border:none;padding:10px 20px;cursor:pointer;font-size:14px;font-weight:500;color:rgba(255,255,255,0.6);border-bottom:2.5px solid transparent;transition:all 0.2s;}
